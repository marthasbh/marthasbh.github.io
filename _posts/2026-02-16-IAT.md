...
#!/usr/bin/env python3
"""
IAT Matcher - Compare Import Address Table calls against MalAPI database
"""
...
def normalize_api_name(name):
    """
    Normalize API name for case-insensitive comparison.
    Also handles common variations like with/without 'A' or 'W' suffix.
    """
...
def find_matches(user_apis):
    """
    Find matches between user-provided API calls and the MalAPI database.
    
    Args:
        user_apis: List of API names to check
        
    Returns:
        Dictionary mapping category to list of matched APIs
    """
    matches = {}
...
def print_matches(matches):
    """
    Pretty print the matches organized by category.
    """
    if not matches:
...
def main():
    ...
    try:
        while True:
            ...
            # Handle multiple APIs on one line (comma or space separated)
            if ',' in line:
